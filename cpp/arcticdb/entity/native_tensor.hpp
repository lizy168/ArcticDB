...
struct NativeTensor {
    ...
    std::span<T> span(const size_t offset = 0, const size_t count = std::dynamic_extent) {
        const std::span<const T> as_const = const_cast<NativeTensor&>(std::as_const(*this)).span<T>(offset, count);
  ...
  private:
    template<typename T>
    void validate_can_take_span(const size_t offset, const size_t count) const {
